...
本文记录冻结旧 reader 的实现和当时验证。当前工具已接通完整角色/世界 DTO 与可选 SQLite 离线导入，详见 [完整离线导入](V10-world-dto-import.zh-CN.md)；游戏后端尚未切换。
...
后续已完成完整 schema 转换、已导入标记与全档原子事务、重复迁移与中断回滚。首次运行时恢复和游戏保存入口切换仍待完成，当前游戏仍使用 v9 快照存储。
...
后续进展：角色与世界均已接入转换，审计报告 profilesConverted/worldConverted/finalSchemaConverted；默认只读，-Import 在新隔离目录内写库并验证。上述历史验证结果保留为 reader 阶段记录，最新结果见完整离线导入文档。
