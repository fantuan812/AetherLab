...
[ordered]@{schema=1;authoredAtUtc=[DateTime]::UtcNow.ToString('o');sourceCommit=(& git -C $taskRoot rev-parse HEAD);backend=$Backend;logs=$taskRun;validation='pending'}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $taskRun 'result.json') -Encoding utf8
