...
$taskSummary=[ordered]@{
    ...
    Command=@(); Cases=@(); Passed=0; Failed=0; Skipped=0; NotRun=@('Scale4096','rendered-play','full-RX-acceptance','cook-hlod','four-player-stress','V8-07-network-acceptance')
}
...
try {
    ...
    foreach($taskCheck in @(@{Enabled=$World;Script='CheckReactions.ps1';Id='world-reactions'},@{Enabled=$Network;Script='TestV8NetworkBaseline.ps1';Id='network-baseline'},@{Enabled=$Services;Script='CheckServices.ps1';Id='world-services'})){
        if(!$taskCheck.Enabled){$taskSummary.NotRun+=$taskCheck.Id;continue}
