param([string]$EngineRoot='C:\Program Files\Epic Games\UE_5.8',[int]$Port=7797,[switch]$DataDefinitions)
...
$taskExtra=@()
if($DataDefinitions){
    $taskFixture=$taskPrefix+'.json'
    New-Item -ItemType Directory -Path (Join-Path $taskRoot 'Saved/Automation') -Force | Out-Null
    Copy-Item -LiteralPath (Join-Path $taskRoot 'Docs/Fixtures/V802.rules.json') -Destination (Join-Path $taskRoot "Saved/Automation/$taskFixture")
    $taskExtra=@("-AetherRulesFixture=$taskFixture",'-AetherV802Net')
}
$taskServer=$null;$taskClients=@();$taskLogs=@()
try {
    $taskServerLog=Join-Path $taskRoot "Saved\Logs\$taskPrefix-Server.log"
    $taskServer=Start-Process -FilePath $taskEditor -ArgumentList (@($taskProject,'/Engine/Maps/Entry?game=/Script/AetherLab.AetherFrontierMode','-server','-game','-AetherV4NetServer',"-AetherSavePrefix=$taskPrefix","-port=$Port",'-nullrhi','-nosound','-unattended','-nop4',"-abslog=$taskServerLog")+$taskExtra) -PassThru -WindowStyle Hidden
    ...
    foreach($taskProfile in 'Alpha','Beta') {
        ...
        $taskClients+=Start-Process -FilePath $taskEditor -ArgumentList (@($taskProject,"127.0.0.1:$Port`?DevProfile=$taskProfile",'-game','-AetherV4NetClient','-nullrhi','-nosound','-unattended','-nop4',"-abslog=$taskLog")+$taskExtra) -PassThru -WindowStyle Hidden
    }
    $taskDeadline=(Get-Date).AddSeconds(60)
    ...
    if($DataDefinitions){foreach($taskLog in $taskLogs){if(!(Select-String -LiteralPath $taskLog -Pattern 'AETHER_V802_EQUIPMENT_PASS' -Quiet)){throw "New item replication failed: $taskLog"}}}
    # These checks do not inspect the other player's replicated profile or late-join reconstruction.
    if($DataDefinitions){Write-Output "V802 new item replication PASS for both clients; isolated fixture $taskFixture"}
