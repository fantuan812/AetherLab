param([string]$EngineRoot='C:\Program Files\Epic Games\UE_5.8',[int]$Port=7812)
...
$taskPrefix='AetherV10Trade_'+[guid]::NewGuid().ToString('N').Substring(0,12)
...
function Start-ClosureProcess([string]$Name,[string[]]$Extra){
 ...
 $taskProc=Start-Process -FilePath $taskEditor -ArgumentList (@($taskProject)+$Extra+@('-AetherV10TradeNetwork','-AetherKeepRegionsLoaded','-nullrhi','-nosound','-unattended','-nop4','-ExecCmds="t.MaxFPS 30"',"-abslog=$taskLog")) -PassThru -WindowStyle Hidden
...
try {
 ...
 if(!(Select-String -LiteralPath $taskServer.Log -Pattern 'V10_TRADE_NETWORK_PASS' -Quiet)){throw 'Trade network check failed.'}
 foreach($taskLog in $taskLogs){
  if(Select-String -LiteralPath $taskLog -Pattern 'V807_CASE FAIL|V807_CLIENT_ACK FAIL|V10_TRADE_CLIENT_FAIL|V10_TRADE_NETWORK_FAIL|V807_FAIL|Fatal error:' -Quiet){throw "Failure: $taskLog"}
 }
 # 逐一要求每个客户端的五个阶段通过，不能只相信服务器汇总日志。
 foreach($taskClient in @($taskAlpha,$taskBeta)){
  foreach($taskPhase in 1..5){
   if(!(Select-String -LiteralPath $taskClient.Log -Pattern "V10_TRADE_CLIENT_PASS id=\w+ phase=$taskPhase " -Quiet)){throw "Missing phase $taskPhase in $($taskClient.Log)"}
  }
 }
 $taskResult=[ordered]@{
  ...
  Passed=@('Distinct merchant session tokens','Duplicate remote buy debits once','Close revokes server authorization','Original receipt replays after close','New command with expired token rejected','Owner-only profile state')
  NotRun=@('Listen mode','Loss/latency injection','Four players','Streaming unload','Restart recovery','Shipping package');Logs=@($taskLogs)
 }
 $taskResult | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $taskRoot 'Saved/Automation/V10-TradeNetwork-Result.json') -Encoding utf8
 Write-Output "V10 merchant dedicated two-client check PASS; logs $taskPrefix"
} finally {
